...
export default function OTP({ otpLength = 6,handleOtpSubmit }) {
  ...
  useEffect(() => {
    if (otpFields.every((digit) => digit !== "")) {
      const otp = otpFields.join("");
  
      handleOtpSubmit({ otp, userId: sessionStorage.getItem("userId") });
    }
  }, [otpFields]);
    ...
    <div className="container">
      {otpFields.map((value, index) => (
        <input
          ...
          type="text"
          maxLength={1}
          className="otp-input w-8 h-8 text-center mr-2 b-2 border"
